...
to assit users. [Schema.org](https://schema.org) allows us to attach higher level nouns on to our semantic HTML
to express "objects" as data, we have even created a horrendus JSON format to help developers encode this schema
data into the web.
...
The goals of these projects are noble, but if


We need to be able to express actions on the data on our pages. I had visions that web intents might be able
to be extended to handle this, but at the time I was simplying stating that a page could perform an action on
a certain piece of data, and if you needed to access that functionality you would delegate the request to "edit
this image" out to another application. But what uf you could isolate function in your page directly, imagine
...
We need to get back to composibility of the web. Natively embeddig microcontent from web into Apps. Micro Web Views.
