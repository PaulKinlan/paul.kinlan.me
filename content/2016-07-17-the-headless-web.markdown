...
I think the opportunities are broadly in two areas:

1. Local to user services &mdash; Data and content powered by the context of the users' device is surfaced to the user
   and they can interact with it without the browser all delivered via HTML and JavaScript (primarily Service Worker). 
2. Remote to user services &mdash; Data and content is powered by aggregating information on the web as a whole and
   running it through an engine that can understand the page content but also the logic that is being executed too.
...
the users are and usable in what ever UserAgent they use.
