...
I don't think I am out of touch with web development, but I certainly missed a trick to feel the pain of the ecosystem.  On one hand Google is telling you to meet your Core Web Vitals, but on the other hand the tooling is an absolute massive pain.
...
The modern image has gone from a simple `<img>` element, to requiring the author to think about the size of the image (in bytes); the size of the image (in resolution) for the target density; The codec being used (remembering the days of JPEG unless you want transparecy) because support isn't even across the web; should the image be lazy loaded and pre-fetched?
...
I don't think you should have to buy a book just to put an image on the page, I also don't want to complain without helping to fix the situation.

So I built a simple tool that can help you for a large number of your image use-cases.
...
Setting the `height` and `width` is required for lazy loading and to ensure that we have 