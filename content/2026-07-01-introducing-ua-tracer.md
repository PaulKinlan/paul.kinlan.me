...
**One caveat first: a User-Agent string is trivially spoofable.** The tool also checks each request's source IP against the CIDR ranges its operator publishes. Every major crawler operator now publishes a list you can match against: [Google](https://developers.google.com/search/docs/crawling-indexing/verifying-googlebot), [OpenAI's GPTBot and OAI-SearchBot](https://platform.openai.com/docs/bots/), [Bing](https://www.bing.com/toolbox/bingbot.json), and [Anthropic's ClaudeBot](https://support.claude.com/en/articles/8896518) ([IP list](https://claude.com/crawling/bots.json)). So a "Googlebot" or "ClaudeBot" trace is either *verified* (the IP is in the operator's published range) or flagged as likely spoofed. The one exception in the data so far is OpenAI's [ChatGPT-User](https://platform.openai.com/docs/bots/), which is triggered by a person rather than run on a schedule and publishes no fixed ranges, so its User-Agent alone can't be confirmed.
...
**Googlebot is documented to render JavaScript in a second pass** ([Google's JavaScript SEO docs](https://developers.google.com/search/docs/crawling-indexing/javascript/javascript-seo-basics) describe crawl → render queue → a headless Chromium executing the script later). I have not caught that render pass in the traces yet; what I have seen is the fetch-only first stage. So "Googlebot doesn't run JS" would be the wrong conclusion to draw, and the tool is set up to record the second stage if and when it arrives. The same applies to **Applebot** ([Apple's docs](http://www.apple.com/go/applebot) say it "may render the content of your website within a browser"); the seven Applebot traces I have are IP-verified real but fetch-only so far.
...
- **Google** publishes its crawler IP ranges ([`common-crawlers.json`](https://developers.google.com/static/crawling/ipranges/common-crawlers.json)) and recommends a reverse-DNS check: resolve the IP to a `googlebot.com` hostname, then forward-resolve it back to the same IP ([Verify Googlebot](https://developers.google.com/search/docs/crawling-indexing/verifying-googlebot)). Googlebot also runs JavaScript in a separate render pass using an evergreen Chromium ([JavaScript SEO basics](https://developers.google.com/search/docs/crawling-indexing/javascript/javascript-seo-basics)).
- **OpenAI** publishes ranges for GPTBot ([`gptbot.json`](https://openai.com/gptbot.json)) and OAI-SearchBot ([`searchbot.json`](https://openai.com/searchbot.json)); overview at [platform.openai.com/docs/bots](https://platform.openai.com/docs/bots/). ChatGPT-User is triggered by a person and has no fixed ranges.
- **Bing** publishes [`bingbot.json`](https://www.bing.com/toolbox/bingbot.json) and a [verification guide](https://www.bing.com/webmasters/help/how-to-verify-bingbot-3b1e6b56) using reverse-DNS to a `search.msn.com` hostname.
- **Apple** publishes Applebot CIDRs at [`search.developer.apple.com/applebot.json`](https://search.developer.apple.com/applebot.json) and a reverse-DNS check against `*.applebot.apple.com` ([About Applebot](http://www.apple.com/go/applebot)). Apple notes Applebot may render pages in a browser, like Googlebot. (Apple also runs `Applebot-Extended`, which doesn't crawl but controls use of crawled data for model training.)
- **Anthropic** publishes its crawler IPs at [`claude.com/crawling/bots.json`](https://claude.com/crawling/bots.json), covering ClaudeBot, Claude-User and Claude-SearchBot ([help article](https://support.claude.com/en/articles/8896518)). Anthropic notes that IP-based blocking "may not work correctly or persistently guarantee an opt-out", but the list is sufficient to confirm a request genuinely originates from Anthropic.
