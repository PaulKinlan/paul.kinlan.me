...
Some things came back, though not for free. [Full RSS](https://full-rss.paulkinlan-ea.deno.net/) is running again, but only after I found out that KV is no longer something you just get: the new platform wants a database provisioned and attached to the app, and until it has one a top-level `await Deno.openKv()` throws before the app can even start. [bcd-training](https://bcd-training.paulkinlan-ea.deno.net/) needed two fixes, neither of them mine: the old `std/http` `serve` helper hardcodes port 8000 where the platform now assigns one, and `@mdn/browser-compat-data` had quietly dropped the default export my `@deno-types` line depended on, so I had to pin a version from 2023 to get a build at all. The old config format does not parse either.

Some things did not come back. The baseline demo, the button classifier from the Fresh and TensorFlow post, and superduperfeeder, my WebSub hub, are all still down. The code is on GitHub. The running versions are not, and a demo that you cannot click is a screenshot with extra steps.
