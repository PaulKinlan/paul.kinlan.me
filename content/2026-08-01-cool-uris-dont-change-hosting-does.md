...
The button classifier from the Fresh and TensorFlow post could not be fixed at all, only rebuilt. It was written against Fresh 1 in 2023, and Fresh 1 starts its own server on a port it chooses, which the new platform will not route to. There is no flag for that. So it is now a [Fresh 2 app](https://is-it-a-button-web-app.paulkinlan-ea.deno.net/): new entry point, new build step, no manifest file, the document shell moved into a layout, and the framework's own upgrade guide is written for a version of Fresh 2 that no longer matches what the scaffolder generates. The model and the markup are the same. Everything holding them up is different.

superduperfeeder, my WebSub hub, I have retired rather than migrated. It leans on queues, and I would rather delete it than pretend it still works.
